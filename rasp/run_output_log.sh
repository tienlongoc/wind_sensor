...
if ! crontab -l | grep -q run_output_log.sh; then
    crontab -l > curr_cron
    echo '00 00 * * 0 /home/pi/ws/wind_sensor/rasp/run_output_log.sh >> /home/pi/ws/wind_sensor/rasp/log.txt' >> curr_cron
    crontab curr_cron
    rm curr_cron
fi
if ! crontab -l | grep -q post_output_log.sh; then
    crontab -l > curr_cron
    echo '* 00 * * 0 /home/pi/ws/wind_sensor/rasp/post_output_log.sh >> /home/pi/ws/wind_sensor/rasp/log/txt' >> curr_cron
    crontab curr_cron
    rm curr_cron
fi
